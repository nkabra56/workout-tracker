...
async function syncNow() {
  ...
  syncBusy = true;
  try {
    const conflicts = await sync(syncToken);
    await load();
...
setInterval(() => {
  if (syncToken && navigator.onLine) syncNow();
}, 30000);
