...
function chart(raw, average, unit, esc) {
  if(!raw.length)return '<p class="empty-progress">No dated measurements in this period. Your first log starts the chart.</p>';
  const points=[...raw,...average], values=points.map(p=>p.value), lo=Math.min(...values), hi=Math.max(...values), pad=Math.max((hi-lo)*.15,.5), min=lo-pad,max=hi+pad;
  ...
  return `<svg class="progress-chart" viewBox="0 0 330 174" role="img" aria-label="Recorded ${esc(unit)} measurements from ${raw[0].date} to ${raw.at(-1).date}. Dots are actual entries; rings are available seven-day averages. No missing measurements are filled."><line x1="48" x2="312" y1="132" y2="132" class="chart-grid"/><line x1="48" x2="312" y1="26" y2="26" class="chart-grid"/><text x="3" y="31">${fmt(max)}</text><text x="3" y="136">${fmt(min)}</text><text x="48" y="161">${raw[0].date.slice(5)}</text>${end!==start?`<text x="312" y="161" text-anchor="end">${raw.at(-1).date.slice(5)}</text>`:''}${raw.map(p=>`<circle cx="${x(p)}" cy="${y(p)}" r="3.5" class="chart-point"><title>${p.date}: ${fmt(p.value)} ${esc(unit)}</title></circle>`).join('')}${average.map(p=>`<circle cx="${x(p)}" cy="${y(p)}" r="5.5" class="chart-average"><title>${p.date}: seven-day mean ${fmt(p.value)} ${esc(unit)} from ${p.count} recorded days</title></circle>`).join('')}</svg>`;
}
export function renderProgress({sessions,weighIns,range,unit,exerciseKey,draft,today,esc}) {
  ...
  const latest=series.raw.at(-1), prior=series.raw[0], selected=group?.rows.at(-1), previous=group?.rows.at(-2), validRows=group?.rows.filter(r=>r.top)||[];
  ...
  return `<div class="page-title"><div><p class="eyebrow">THE LONG VIEW</p><h1>Progress</h1></div><label class="progress-filter">Period<select id="progress-range">${[[7,'7 days'],[30,'30 days'],[90,'90 days'],[0,'All time']].map(([n,label])=>`<option value="${n}" ${n===days?'selected':''}>${label}</option>`).join('')}</select></label></div>
  <article class="progress-card"><div class="section-title"><h2>Body weight</h2><label class="progress-filter">Display<select id="progress-unit"><option ${unit==='lb'?'selected':''}>lb</option><option ${unit==='kg'?'selected':''}>kg</option></select></label></div><p>Separate from the weights you lift.</p>
